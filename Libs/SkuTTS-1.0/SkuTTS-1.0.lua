...
function SkuTTS:ToggleAutoRead(aEngine, aReset)
	...
	if SkuTTS.MainFrame:IsVisible() == true then
		if SkuTTS.AutoReadMode ~= true then
			...
			SkuTTS.AutoReadMode = nil
			SkuTTS.AutoReadEventFlag = nil
			--SkuMenu.Voice:StopOutputEmptyQueue()
		end
	end
end
...
function SkuTTS:NextLink(aEngine, aReset)
	if SkuTTS.MainFrame:IsVisible() == true then
		SkuTTS:ReadLinkNumber(SkuMenu.currentMenuPosition.linksSelected, aReset, aEngine)
	end
end
---------------------------------------------------------------------------------------------------------------------------------------
...
function SkuTTS:ReadLinkNumber(aLinkNumber, aNoReset, aEngine)
	...
	if SkuTTS.MainFrame:IsVisible() == true then
		if (SkuMenu.currentMenuPosition.links) then
			if SkuMenu.currentMenuPosition.links[aLinkNumber] then
				--if not aEngine then
					--SkuMenu.Voice:OutputStringBTtts(SkuMenu.currentMenuPosition.links[aLinkNumber], aNoReset, true)
				--else
					SkuMenu.Voice:OutputStringBTtts(SkuMenu.currentMenuPosition.links[aLinkNumber], aNoReset, true, nil, nil, false, nil, 1)
				--end
			end
		end
	end
...
function SkuTTS:IsLinkInLinkList(aLinkName, aLinkList)
	...
	if not SkuMenu.currentMenuPosition then
		return
	end
	if SkuMenu.currentMenuPosition.currentLinkName then
		if SkuMenu.currentMenuPosition.currentLinkName ~= "" then
			if slower(SkuMenu.currentMenuPosition.currentLinkName) == slower(aLinkName) then
				return true
			end
		end
	end
	...
	if SkuTTS.MainFrame:IsVisible() == true then
		...
		if (sections[aSectionNumber]) then
			if sections[aSectionNumber][aLineNumber] then
				...
				if not SkuMenu.currentMenuPosition then
					SkuMenu.currentMenuPosition = {}
				end

				local tNoLinksPlease = SkuMenu.currentMenuPosition.parent
				if tNoLinksPlease then
					tNoLinksPlease = SkuMenu.currentMenuPosition.parent.name ~= L["All entries"]
				end

				if (aSectionNumber ~= 1 or aLineNumber ~= 1) == true or (not SkuMenu.currentMenuPosition.linksHistory and tNoLinksPlease == true) == true then
					if SkuMenu.currentMenuPosition.linksHistory then
						...
						SkuMenu.currentMenuPosition.links = SkuTTS:GetLinksTableFromString(sections[aSectionNumber][aLineNumber], sections[1][1], true)
					else
						SkuMenu.currentMenuPosition.links = SkuTTS:GetLinksTableFromString(sections[aSectionNumber][aLineNumber], sections[1][1])
					end

					if #SkuMenu.currentMenuPosition.links > 0 then
						if SkuMenu.db.profile["SkuAdventureGuide"].links.enableLinksInTooltips == true then
							if SkuTTS:IsAutoRead() ~= true then
								...
								if SkuMenu.db.profile["SkuAdventureGuide"].links.tooltipLinksIndicator ~= "sound" then
									sections[aSectionNumber][aLineNumber] = tOrg.."§".." : "..L["Links:"]..";"
								else
									SkuAdventureGuide:PlaySound(SkuAdventureGuide.HistoryNotifySounds[SkuMenu.db.profile["SkuAdventureGuide"].history.soundOnNewLinkInHistory])
									sections[aSectionNumber][aLineNumber] = tOrg.."§; ;"
								end
								for x = 1, #SkuMenu.currentMenuPosition.links do
									sections[aSectionNumber][aLineNumber] = sections[aSectionNumber][aLineNumber]..SkuMenu.currentMenuPosition.links[x]..";"
								end
							else
								if string.find(sections[aSectionNumber][aLineNumber], "§") then
				...
				--if not aEngine then
					--SkuMenu.Voice:OutputStringBTtts(tCleanOutput, aNoReset, true)
				--else
					SkuMenu.Voice:OutputStringBTtts(tCleanOutput, aNoReset, true, nil, nil, false, nil, 1)
				--end
			end
		end
	end
...
function SkuTTS:Output(text, duration)
	...
	if duration > 0 then
		if SkuTTS.MainFrame:IsVisible() == false then
			--SkuMenu.Voice:OutputString("sound-on3_1")
			SkuMenu.Voice:OutputStringBTtts("sound-on3_1", true, true, 0.2, true)
...
function SkuTTS:Hide()
	...
	if SkuTTS.MainFrame:IsVisible() == true then
		SkuMenu.Voice:StopOutputEmptyQueue(true, nil)
		SkuMenu.Voice:OutputStringBTtts("sound-off2", true, true, 0.2)
	end
	SkuTTS.MainFrame:Hide()
	SkuTTS.AutoReadMode = nil
end
