...
local L = LibStub("AceLocale-3.0"):GetLocale("SkuMenu", false)
...
function SkuVoice:Create()
		...
		if aEventName == "VOICE_CHAT_TTS_PLAYBACK_FINISHED" then
			if IsMacClient() ~= true then
				...
				if destination == 1 then
					...
					if SkuVoice.TutorialPlaying == 0 then
						--SkuMenu.Voice:OutputString("sound-TutorialClose01", false, false, 0.3, true)
						--SkuMenu.Voice:OutputString("sound-waterdrop1", false, false, 0.3, true)
						dprint("STOP")
	...
	C_Timer.After(0.01, function()
		--C_VoiceChat.SpeakText(SkuMenu.db.profile["SkuChat"].WowTtsVoice - 1, " ", 4, SkuMenu.db.profile["SkuChat"].WowTtsSpeed, SkuMenu.db.profile["SkuChat"].WowTtsVolume)
	end)
		...
		if fTimeBTTS > 0.01 then
			...
			if #mSkuVoiceQueueBTTS > 0 then
				...
				if tValue == "queuereset" and SkuVoice.TutorialPlaying == 0 then
					table.remove(mSkuVoiceQueueBTTS, 1)
					--if SkuMenu.db.profile["SkuChat"].neverResetQueues ~= true then
					...
					if #mSkuVoiceQueueBTTS > 1 or tLastWait <= 0 then
						...
						if not tIsAlreadySpeakingThat then
							...
							local tUttId
							if tIsTutorial then
								--tUttId = C_VoiceChat.SpeakText(SkuMenu.db.profile["SkuChat"].WowTtsVoice - 1, tValue, 1, SkuMenu.db.profile["SkuChat"].WowTtsSpeed, SkuMenu.db.profile["SkuChat"].WowTtsVolume)
							else
								tUttId = C_VoiceChat.SpeakText(SkuMenu.db.profile["SkuMenu"].WowTtsVoice - 1, tValue, 4, SkuMenu.db.profile["SkuMenu"].WowTtsSpeed, SkuMenu.db.profile["SkuMenu"].WowTtsVolume)
							end
...
local function SplitStringBTTS(aString)
	...
	--[[
	if SkuMenu.db.profile["SkuChat"].doNotReadoutEmojis ~= true then
...
function SkuVoice:OutputStringBTtts(aString, aOverwrite, aWait, aLength, aDoNotOverwrite, aIsMulti, aSoundChannel, engine, aSpell, aVocalizeAsIs, aInstant, aDnQ, aIgnoreLinks, aIsTutorial)
	print("OutputStringBTtts", aString, aOverwrite, aWait, aLength, aDoNotOverwrite, aIsMulti, aSoundChannel, engine, aSpell, aVocalizeAsIs, aInstant, aDnQ, aIgnoreLinks, aIsTutorial)
	if not aString then
		return
	end
	...
	--empty the queue
	--if aOverwrite == true and SkuMenu.db.profile["SkuChat"].neverResetQueues ~= true then
		if SkuVoice.TutorialPlaying == 0 then
...
--[[
	...
	for x = 1, #tStrings do
		print(tStrings[x])
		--if SkuMenu.db.profile["SkuChat"].WowTtsTags ~= false and aIsTutorial ~= true then
	...
	--tFinalStringForBTts = '<voice required="Language='..SapiLangIds[SkuMenu.Loc]..'">'..tFinalStringForBTts..'</LANG>'
	--tFinalStringForBTts = '<LANG LANGID="'..SapiLangIds[SkuMenu.Loc]..'">'..tFinalStringForBTts..'</LANG>'
	--[[
	if SkuMenu.db.profile["SkuChat"].WowTtsTags ~= false and aIsTutorial ~= true then
	...
	if aIsTutorial == true and tFinalStringForBTts ~= "" then
		...
		--[[
		if SkuMenu.db.profile["SkuChat"].WowTtsTags ~= false then
	...
	if SkuVoice.TutorialPlaying == 0 or (SkuVoice.TutorialPlaying > 0 and aIsTutorial == true) then
		...
		if IsMacClient() == true then
			...
			if not aIgnoreLinks then
				SkuMenu.TTS:GetLinksTableFromString(tFinalStringForBTtsMac, "")
			end
		else
			if aInstant then
			...
			if not aIgnoreLinks then
				SkuMenu.TTS:GetLinksTableFromString(tFinalStringForBTts, "")
			end
		end
	end
...
function SkuVoice:OutputString(aString, aOverwrite, aWait, aLength, aDoNotOverwrite, aIsMulti, aSoundChannel, engine, aSpell, aVocalizeAsIs, aInstant, aDnQ, aIgnoreLinks, aIsTutorial, aAudioFile) -- for strings with lookup in string index
	print("OutputString", aString, aOverwrite, aWait, aLength, aDoNotOverwrite, aIsMulti, aSoundChannel, engine, aSpell, aVocalizeAsIs, aInstant, aDnQ, aIgnoreLinks, aIsTutorial, aAudioFile)
	if not aString then
		return
	end
	...
	if not tIsSound then
		...
		if not (string.find(aString, "sound%-") or string.find(aString, "male%-") or string.find(aString, "brian%-") or string.find(aString, "emma%-")) and SkuMenu.db.profile["SkuChat"].allChatViaBlizzardTts == true then
			print("redirecting to BTTS", aString)
		...
		aSoundChannel = aSoundChannel or "Talking Head"
	...
	if engine then
		...
		if engine ~= 3 then
			if IsMacClient() == true then
				...
				mSkuVoiceQueueBTTS_Speaking = {}
				table.insert(mSkuVoiceQueueBTTS_Speaking, tValue)
				C_VoiceChat.SpeakText(SkuMenu.db.profile["SkuChat"].WowTtsVoice - 1, aString, 4, SkuMenu.db.profile["SkuChat"].WowTtsSpeed, SkuMenu.db.profile["SkuChat"].WowTtsVolume)
				if not aIgnoreLinks then
					SkuMenu.TTS:GetLinksTableFromString(aString, "")
				end
			else
				C_VoiceChat.StopSpeakingText()
				mSkuVoiceQueueBTTS_Speaking = {}
		...
		if not tIsSound then
			...
			--collect links
			if not aIgnoreLinks then
				SkuMenu.TTS:GetLinksTableFromString(aString:gsub(";", " "), "")
			end
		end

		local tStrings = {}
		if (string.find(aString, "sound-") or string.find(aString, "male%-") or string.find(aString, "brian%-") or string.find(aString, "emma%-")) then
			print("  sound or emma/brian found", aString)
			local a, b = string.find(aString, "sound-")
			print("   a. b. 1", a, b)
			if not b then
				a, b = string.find(aString, "male%-")
			...
			if b == nil then
				...
				for x = 1, #tSplittedString do
					table.insert(tStrings, tSplittedString[x])
					print("splitted", x, tSplittedString[x])
				end
			end

		else
			print("  no sound or emma/brian found")
		...
		for x = 1, #tStrings do
			...
			if not tIsSound then
				...
				if tFile == nil then
					...
					if SkuMenu.db then
						if SkuMenu.db.realm.missingAudio == nil then
							SkuMenu.db.realm.missingAudio = {}
						end
						if not SkuMenu.db.realm.missingAudio[tStrings[x]] then
							SkuMenu.db.realm.missingAudio[tStrings[x]] = 1
						else
							SkuMenu.db.realm.missingAudio[tStrings[x]] = SkuMenu.db.realm.missingAudio[tStrings[x]] + 1
						end
					end
				end
			else
			...
			print("    tFile, tPath, tLength", tFile, tPath, tLength)
			...
			if tFile then
				if tFile ~= "" then
					...
					--if isMulti == true then
						--tLength = tLength - ((100 - tonumber(SkuMenu.db.profile["SkuMenu"].TTSSepPause)) / 100) -- - 0.15
					--end
					...
					print("     insert to queue", aInstant, tStrings[x], tFile, aWait, tLength, aDoNotOverwrite, aSoundChannel, aDnQ)
...
function SkuVoice:GetAudiodata(aString)
	...
	if SkuAudioFileIndexIntegrated[SkuMenu.Loc][aString] ~= nil then
		tFile = SkuAudioFileIndexIntegrated[SkuMenu.Loc][aString]
		tPath = [[Interface\AddOns\SkuMenu\SkuAudioData\assets\audio\]]..SkuMenu.Loc..[[\]]
		tLen = SkuAudioDataLenIndexIntegrated[SkuMenu.Loc][SkuAudioFileIndexIntegrated[SkuMenu.Loc][aString]]
	end
	
	if tFile == nil then
		--if SkuAudioFileIndex[aString] ~= nil then
			--tFile = SkuAudioFileIndex[aString]
			--tPath = [[Interface\AddOns\]]..SkuMenu.AudiodataPath..[[\assets\audio\]]
